...
def get_metadata_czi(filename, dim2none=False,
    ...
    metadata = checkdims_czi(aicsczi, metadata)

    # determine pixel type for CZI array using aicspylibczi
    metadata['Pixeltype_aics'] = aicsczi.pixel_type
    # determine pixel type for CZI array
    metadata['NumPy.dtype'] = get_dtype_fromstring(aicsczi.pixel_type)
...
def replace_value(data, value=0):
    """Replace specific values in array with NaN
...
def get_dimpositions(dimstring, tocheck=['B', 'S', 'T', 'Z', 'C', 'M', 'H', 'Y', 'X']):
...
def get_czi_planetable(czifile):
    ...
    md, add = get_metadata_czi(czifile)
...
def getbboxes_allscenes(czi):

    all_bboxes = []
    if 'S' in czi.dims:
        numscenes = czi.get_dims_shape()[0]['S'][1]
    else:
        numscenes = 1
...
def get_shape_allscenes(czi):
    ...
    #if same_shape:
    if czi.shape_is_consistent:
        array_size_all_scenes = shape_single_scenes[0].copy()
        array_size_all_scenes[0] = numscenes
...
def read_czi_scene(czi, scene, metadata,
    ...
    if array_type == 'numpy':
        # create the required array for this scene as numpy array
    ...
    # check if scale factor has a reasonable value
    if scalefactor < 0.01 or scalefactor > 1.0:
        print('Scalefactor too small or too large. Will use 1.0 as fallback')
        scalefactor = 1.0
